...
async function seedDatabase() {
  ...
  try {
    ...
    console.log('Manager user created/verified');

    // 3. Create Supervisor Users
    ...
    console.log('Supervisor users created/verified');
    ...
    console.log('Worker users created/verified');
    ...
    console.log('Projects created/verified');
    ...
    for (const site of sites) {
      ...
      if (result.rows[0]) {
        ...
        const existing = await client.query('SELECT id FROM sites WHERE name = $1 AND project_id = $2', [site.name, site.project_id]);
        if (existing.rows[0]) {
          siteIds.push(existing.rows[0].id);
        }
      }
    }
    console.log('Sites created/verified');

    // 7. Assign Workers to Sites
    console.log('Assigning workers to sites...');
    ...
    console.log('Materials created/verified');

    // 9. Create Material Inventory
    ...
    console.log('Material inventory created');
    ...
    console.log('Equipment created/verified');
    ...
    // 12. Create Sample Material Transactions
    ...
    for (let i = 0; i < siteIds.length; i++) {
      ...
      for (let j = 0; j < 3; j++) {
        ...
        await client.query(
          `INSERT INTO material_transactions (site_id, material_id, transaction_type, quantity, created_by)
           VALUES ($1, $2, 'delivery', $3, $4)`,
          [siteId, materialId, quantity, managerId]
        );
      }
    }
    console.log('Material transactions created');

    // 13. Create Sample Attendance Records (Last 7 days)
    ...
    for (let day = 0; day < 7; day++) {
      ...
      for (const workerId of workerIds) {
        const shouldBePresent = Math.random() > 0.2; // 80% attendance rate
        ...
        if (shouldBePresent) {
          ...
          await client.query(
            `INSERT INTO attendance (user_id, site_id, attendance_date, clock_in, clock_out, hours_worked, status, marked_by)
             VALUES ($1, $2, $3, $4, $5, $6, 'present', $7)
             ON CONFLICT (user_id, attendance_date, site_id) DO NOTHING`,
            [workerId, siteIds[Math.floor(Math.random() * siteIds.length)], dateStr, clockIn, clockOut, hoursWorked, supervisor1Id]
          );
        }
      }
    }
    ...
    console.log('\nSupervisor:');
    console.log('  Username: supervisor1 / supervisor2');
    console.log('  Password: supervisor123');
    console.log('\nWorker:');
    console.log('  Username: worker1, worker2, worker3, worker4, worker5');
