...
async function resetAndSeed() {
  ...
  try {
    ...
    // Now import and run initialization
    const { initializeDatabase } = await import('../config/initDatabase');
    await initializeDatabase();
    
    console.log('Schema created. Starting seeding...');
    await seedDatabase();
...
async function seedDatabase() {
  ...
  try {
    ...
    const supervisor1Id = supervisor1Result.rows[0].id;
    ...
    const supervisor2Id = supervisor2Result.rows[0].id;
    ...
    // Create Sample Material Transactions
